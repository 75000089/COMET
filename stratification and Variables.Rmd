...
#"Company"            "Study_id"           "Toxin"              "Target"                      "Animal_no."         "Timepoint"          "Sacrifice_subgroup"        "doseFlag" 
...
```{r}
MNARbias1 <- cbind(rn,rep("incomplete", times = 5),missedBias, rep(0, times = 5), rep("MNAR", times = 5), rep("imputedBias.1"))

MNARbias2 <- cbind(rn,rep("incomplete", times = 5),missedBias2, rep(0, times = 5), rep("MNAR", times = 5), rep("imputedBias.2"))
...
# 
# list_name <- paste("MICErf", missing, impco, sep = "")
# assign(list_name, MICErf, envir = .GlobalEnv)
...
# 
# list_name <- paste("missForest", missing, impco, sep = "")
# assign(list_name, missForest, envir = .GlobalEnv)
...
# # Assuming MICErf_strata_MCAR_Company is your list with 20 entries
# n_entries <- length(results_list)
# 
# # Initialize a matrix to store the results
# result_matrix_NRMSE <- matrix(NA, nrow = n_entries, ncol = length(results_list[[1]][["NRMSE"]]))
# result_matrix_partialMAE <- matrix(NA, nrow = n_entries, ncol = length(results_list[[1]][["partialMAE"]]))
# result_matrix_fullMAE <- matrix(NA, nrow = n_entries, ncol = length(results_list[[1]][["fullMAE"]]))
# result_matrix_imputedBias.1 <- matrix(NA, nrow = n_entries, ncol = length(results_list[[1]][["imputedBias.1"]]))
# result_matrix_imputedBias.2 <- matrix(NA, nrow = n_entries, ncol = length(results_list[[1]][["imputedBias.2"]]))
# 
# # Loop through the list and extract "NRMSE" values into the matrix
# for (i in 1:n_entries) {
#   result_matrix_NRMSE[i, ] <- unlist(results_list[[i]][["NRMSE"]])
#    result_matrix_partialMAE[i, ] <- unlist(results_list[[i]][["partialMAE"]])
#     result_matrix_fullMAE[i, ] <- unlist(results_list[[i]][["fullMAE"]])
#      result_matrix_imputedBias.1[i, ] <- unlist(results_list[[i]][["imputedBias.1"]])
#       result_matrix_imputedBias.2[i, ] <- unlist(results_list[[i]][["imputedBias.2"]])
#       
# }
# 
# mean_values_NRMSE <- apply(result_matrix_NRMSE, 2, mean)
# std_dev_values_NRMSE <- apply(result_matrix_NRMSE, 2, sd)
# 
# mean_values_partialMAE <- apply(result_matrix_partialMAE, 2, mean)
# std_dev_values_partialMAE <- apply(result_matrix_partialMAE, 2, sd)
# 
# mean_values_fullMAE <- apply(result_matrix_fullMAE, 2, mean)
# std_dev_values_fullMAE <- apply(result_matrix_fullMAE, 2, sd)
# 
# mean_values_imputedBias.1 <- apply(result_matrix_imputedBias.1, 2, mean)
# std_dev_values_imputedBias.1 <- apply(result_matrix_imputedBias.1, 2, sd)
# 
# mean_values_imputedBias.2 <- apply(result_matrix_imputedBias.2, 2, mean)
# std_dev_values_imputedBias.2 <- apply(result_matrix_imputedBias.2, 2, sd)
# 
# means<-list(NRMSE = mean_values_NRMSE,
#             paritalMAE = mean_values_partialMAE,
#             fullMAE = mean_values_fullMAE,
#             imputedBias.1 = mean_values_imputedBias.1,
#             imputedBias.1 = mean_values_imputedBias.2)
...
<!-- ```{r} -->
<!-- # Assuming MICErf_strata_MCAR_Company is your list with 20 entries -->
<!-- n_entries <- length(MICErf_strata_MCAR_Company) -->

<!-- # Initialize a matrix to store the results -->
<!-- result_matrix <- matrix(NA, nrow = n_entries, ncol = length(MICErf_strata_MCAR_Company[[1]][["NRMSE"]])) -->

<!-- # Loop through the list and extract "NRMSE" values into the matrix -->
<!-- for (i in 1:n_entries) { -->
<!--   result_matrix[i, ] <- unlist(MICErf_strata_MCAR_Company[[i]][["NRMSE"]]) -->
<!-- } -->


<!-- tt<-rbind(unlist(MICErf_strata_MCAR_Company[[1]][["NRMSE"]]),  -->
<!--           unlist(MICErf_strata_MCAR_Company[[2]][["NRMSE"]])) -->


<!--  tt <- list(MICErf_strata_MCAR_Company, MICErf_MCAR) -->
<!-- rbind(unlist(tt[[1]][["NRMSE"]]), unlist(tt[[1]][["NRMSE"]])) -->
...
<!-- #make the predictor matrix using all covariates (including clin_chem parameters) -->
<!-- x<-cbind(x,t[["missing 20 %"]][,start:end]) -->

<!-- imp<- list() -->

<!-- companies <- c("D", "L", "N", "R", "S") -->
<!-- for (i in 1:5) { -->
<!--   imp[[i]] <- missForest(xmis = x[x$Company == companies[i], ], ntree = 10, variablewise = FALSE) -->
<!-- } -->

<!-- test <- rbind(imp[[1]][["ximp"]], imp[[2]][["ximp"]], imp[[3]][["ximp"]], imp[[4]][["ximp"]], imp[[5]][["ximp"]]) -->
